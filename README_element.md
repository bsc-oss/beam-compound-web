...
| Command                       | Runs                          |
| ----------------------------- | ----------------------------- |
| `npm run dev`                 | Runs a local Storybook server |
| `npm run lint`                | Lints all components          |
| `npm run gen:component $name` | Bootstraps a new component    |
...
| `npm run e2e:docker`    | Runs end-to-end tests             |
| `npm run e2e:docker -u` | Updates end-to-end test snapshots |
...
2. Run `npm version <patch (x.x.1) | minor (x.1.x) | major (1.x.x)>`.
   The action will:
   - add a commit bumping the version in `package.json`
   - add a tag with that version
   - deploys the version when the tag gets pushed.
3. Manually add a changelog to `CHANGES_PG.md` file.
