...
describe("EditInPlace", () => {
  ...
  it("should show the buttons when the input or buttons are focused", async () => {
    ...
    // PG_CHANGED: The save button is disabled (nothing typed yet), so tab skips it and focuses cancel
    await userEvent.keyboard("{tab}");

    // PG_CHANGED: It should still be visible (cancel button is focused, still within the form)
    expect(queryByRole("button", { name: "Save" })).toBeInTheDocument();
